...
test('every rock cell has an escape direction, so a buried zombie can walk out', () => {
  for (const map of MAPS) {
    const f = new Field(map);
    let checked = 0;
    for (let y = 0; y < f.h; y++) {
...
test('no map has an unreachable pocket of open ground', () => {
  // Zombies standing in a sealed pocket have nowhere to walk, so they mill in place
