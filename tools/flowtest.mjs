...
// leak counter. Every zombie must arrive. Anything left alive after the deadline is
...
const child = spawn(CHROME, [
  ...
  `--user-data-dir=${join(process.env.TEMP ?? '.', 'biomass-flowtest')}`,
  ...
  return {
    ...
    async state() {
      const r2 = await call('Runtime.evaluate', {
        expression: 'JSON.stringify(globalThis.__biomass ? globalThis.__biomass() : null)',
        returnByValue: true,
      });
...
async function testMap(map) {
  ...
  try {
    for (;;) {
      ...
      if (s) {
        last = s;
        // every spawned zombie has either leaked or been recycled: none are stuck
...
console.log(`\n${results.length - bad.length}/${results.length} maps let every zombie reach the base.`);
