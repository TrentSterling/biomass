...
export class Menu {
  ...
  buildMain() {
    ...
    wrap.appendChild(el('h1', 'title', 'BIOMASS'));
    wrap.appendChild(el('div', 'tagline', 'the horde is a fluid'));

    const stats = el('div', 'menustats');
    const total = save.data.totalKills;
    stats.innerHTML = total > 0
      ? `${total.toLocaleString()} zombies killed all time`
      : 'a hundred thousand zombies, all of them on the GPU';
  ...
  buildSettings() {
    ...
    for (const n of [100000, 250000, 500000, 1000000, 2000000]) {
      ...
      o.value = String(n);
      if (save.settings.zombieCap === n) o.selected = true;
      capSel.appendChild(o);
    }
    capSel.onchange = () => {
      save.setSetting('zombieCap', Number(capSel.value));
      this.cb.onSetting('zombieCap', Number(capSel.value));
    };
    capRow.append(el('span', 'slabel', 'Zombie capacity'), capSel, el('span', 'sval', 'next run'));
